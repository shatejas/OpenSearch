...
public abstract class Engine implements LifecycleAware, Closeable {
     ...
     * Acquires a point-in-time reader that can be used to create {@link Engine.Searcher}s on demand.
     */
    public SearcherSupplier acquireSearcherSupplier(Function<Searcher, Searcher> wrapper,
                                                    SearcherScope scope,
                                                    Set<String> criterias) throws EngineException {
...
//        System.out.println("Inc ref called during acquireSearcherSupplier");
        ...
        try {
            ...
            if (criterias != null && !criterias.isEmpty()) {
                indexReader = acquire.getCriteriaBasedReader(criterias);
            } else {
                indexReader = acquire;
            }
    ...
    public Searcher acquireSearcher(String source, IndexWriter writer) throws EngineException {
        try {
...
//                                System.out.println("Dec ref called during Engine failure");
    ...
    public static class Index extends Operation {
        ...
        private final NamespaceFieldMapper namespaceFieldMapper;
        ...
        public Index(
            ...
            this.namespaceFieldMapper = null;
        }
        ...
        public Index(
            ...
            long ifSeqNo,
            long ifPrimaryTerm,
            NamespaceFieldMapper namespaceFieldMapper
        ) {
            super(uid, seqNo, primaryTerm, version, versionType, origin, startTime);
            assert (origin == Origin.PRIMARY) == (versionType != null) : "invalid version_type=" + versionType + " for origin=" + origin;
            assert ifPrimaryTerm >= 0 : "ifPrimaryTerm [" + ifPrimaryTerm + "] must be non negative";
            ...
            this.ifSeqNo = ifSeqNo;
            this.ifPrimaryTerm = ifPrimaryTerm;
            this.namespaceFieldMapper = namespaceFieldMapper;
        }
        ...
        public NamespaceFieldMapper namespaceFieldMapper() {
            return this.namespaceFieldMapper;
        }
